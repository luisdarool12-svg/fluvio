...
from business_hours import closed_weekdays
...
def _fecha_relativa_context(today: date, closed: set[int]) -> str:
    """
    ...
    for wd in range(7):
        ...
        closed_mark = "  ← CERRADO, avísale al cliente" if wd in closed else ""
        today_mark = " (HOY)" if d == today else ""
        lines.append(
            f"    este {_DAYS_ES[wd]:<12} → {d.strftime('%d/%m/%Y')}{today_mark}{closed_mark}"
        )
    ...
    for wd in range(7):
        ...
        closed_mark = "  ← CERRADO, avísale al cliente" if wd in closed else ""
        lines.append(
            f"    el próximo {_DAYS_ES[wd]:<8} → {proximo.strftime('%d/%m/%Y')}{closed_mark}"
        )

    return "\n".join(lines)


def _calendar_context(closed: set[int], days: int = 60) -> str:
    today = datetime.now(TZ).date()
    entries = []
    for i in range(days + 1):
        d = today + timedelta(days=i)
        short = _SHORT_ES[d.weekday()]
        closed_mark = " CERRADO" if d.weekday() in closed else ""
        entries.append(f"{d.strftime('%d/%m/%Y')}({short}{closed_mark})")
    return ", ".join(entries)
