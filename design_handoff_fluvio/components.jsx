...
function Wordmark({ size = 26, business }) {
  return (
    <div className="row gap-10">
      <Logo size={size} />
      <div className="col" style={{ gap: 0 }}>
        <span className="display" style={{ fontWeight: 700, fontSize: size * .72, letterSpacing: '-0.03em', lineHeight: 1 }}>
          {business || 'Fluvio'}
        </span>
        {business && <span style={{ fontSize: 11.5, color: 'var(--ink-3)', fontWeight: 500, marginTop: 2 }}>vía Fluvio</span>}
      </div>
    </div>
  );
}
...
function Avatar({ name, size = 34, src }) {
  ...
  return (
    <div className="avatar" style={{ width: size, height: size, fontSize: fs, background: avatarColor(name) }}>
      {initials(name)}
    </div>
  );
}
...
function RingProgress({ value, size = 56, stroke = 6, color = 'var(--accent)' }) {
