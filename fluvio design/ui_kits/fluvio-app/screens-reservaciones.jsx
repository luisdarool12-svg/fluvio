    ...
    <Scrim onClose={onClose}>
      <div className="drawer">
        <div className="drawer-head">
          <div style={{ width: 38, height: 38, borderRadius: 10, background: 'var(--violet-light)', color: 'var(--plum)', display: 'grid', placeItems: 'center' }}>
        ...
        <div className="drawer-body">
          ...
          <div className="field" style={{ position: 'relative' }}>
            ...
            {showClients && client && (
              <div className="card" style={{ position: 'absolute', top: '100%', left: 0, right: 0, marginTop: 4, zIndex: 5, padding: 6, boxShadow: 'var(--shadow-pop)' }}>
                {matches.length ? matches.map(c => (
                  ...
                  <div className="row gap-10" style={{ padding: '10px 8px', cursor: 'pointer' }} onClick={() => setShowClients(false)}>
                    <div style={{ width: 28, height: 28, borderRadius: 99, background: 'var(--violet-light)', color: 'var(--plum)', display: 'grid', placeItems: 'center' }}><Icon name="plus" size={15} /></div>
            ...
            <div className="row gap-8" style={{ flexWrap: 'wrap' }}>
              {Object.entries(CHANNEL).map(([k, c]) => (
                <button key={k} className={'chip' + (channel === k ? ' chip-on' : '')} onClick={() => setChannel(k)}
                  style={channel === k ? { background: 'var(--violet-light)', color: 'var(--plum)', border: '1px solid var(--violet-light)', fontWeight: 600 } : { cursor: 'pointer' }}>
